...
#[pg_guard]
extern "C-unwind" fn copy_split_startup(dest: *mut DestReceiver, operation: i32, tupledesc: TupleDesc) {
...
#[pg_guard]
extern "C-unwind" fn copy_split_receive(slot: *mut TupleTableSlot, dest: *mut DestReceiver) -> bool {
    let split_parquet_dest = unsafe {
        (dest as *mut CopyToParquetSplitDestReceiver)
            .as_mut()
            .expect("invalid split parquet dest receiver ptr")
